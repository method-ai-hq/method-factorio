...
def main():
    ...
    parser.add_argument("--kind", choices=["development", "final", "baseline"], required=True)
    ...
    try:
        ...
        host_command = [python, str(ROOT / "scripts/search_host.py"), "--run", run_name,
            ...
            "--job-deadline", str(args.deadline), "--port", str(18900+args.lane),
        ...
        with (trial / "host-console.log").open("w") as log:
            ...
            ready = None
            while time.time() < min(started+100, args.deadline-40):
        ...
        record["artifacts"] = {"policy_trace": str(trial / "runtime/method/events.jsonl"), "actions": str(game_run / "actions.jsonl"), "video": str(game_run / "recording/video.mp4"), "save": str(game_run / "game/final.zip"), "inspection": str(game_run / "save-inspection/result.json"), "verdict": str(trial / "verdict.json")}
