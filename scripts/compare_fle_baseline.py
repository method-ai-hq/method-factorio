"""Run matched Astra pilots through a restricted Unix connection to FLE.
...
PORTS = {"C": (18803, 27203, 34303)}
SOURCES = ["policies/iron-plates-v1.method", "docs/control-api.md",
           "scripts/control_test.py", "scripts/check_result.py", "scripts/compare_fle_baseline.py", "docs/control-api-unix.md"]
...
def summarize(run, timing):
    ...
    for action in actions:
        state = action["response"].get("state") or {}
    ...
    timing.update({"first_request_at": actions[0]["started_at"] if actions else None,
                   ...
                   "concurrent_load": "A/B/C lanes plus other environment checks on same Mac"})
...
def episode(lane, seed, batch, env, batch_deadline):
    ...
    socket_path = f"/tmp/factorio-c-{seed}-{int(time.time())}.sock"
    timing = {"baseline": lane, "seed": seed, "start_at": time.time(), "ready_at": None,
              ...
              "model": "gpt-6-astra", "reasoning_settings": "user configuration, same for A/B/C",
              "variant": "C-matched: same FLE dispatcher and post-action state over Unix socket",
              "pilot": seed == 44340}
    worker = host = None
    try:
        with host_log.open("x") as log:
            host = subprocess.Popen([sys.executable, "-u", str(ROOT / "scripts/control_test.py"),
                ...
                "--transport", "unix", "--socket-path", socket_path,
                "--rcon-port", str(rcon), "--game-port", str(udp), "--speed", "1"],
            ...
            doc = str(ROOT / "docs/control-api-unix.md")
            endpoint = "unix:" + socket_path
            inputs = {"endpoint": endpoint, "tool_documentation": doc}
            resources = {"game": {"description": f"Use only newline-delimited JSON over {endpoint}. Tool documentation: {doc}. "
            ...
            prompt = (policy["steps"]["produce"]["do"].replace("Use only HTTP POSTs", "Use only newline-delimited JSON socket requests") + "\n\n" + json.dumps({
                ...
                "\nReturn a short honest final result. Do not use Method; this is the matched FLE connection baseline.\n")
            ...
            with events_path.open("w") as output, (run / "controller-stderr.log").open("w") as errors:
                ...
                if prompt:
                    ...
                    worker.stdin.close()
                limit = min(batch_deadline, time.monotonic() + 720)
