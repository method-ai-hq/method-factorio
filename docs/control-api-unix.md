# Small production test interface: Unix connection

This is a local integration test. It does not test all Factorio actions.

Connect to the path after `unix:` in the supplied endpoint. Send one JSON
object followed by a newline, then read one JSON response line. Use a new
connection for each request. This connection calls the same restricted FLE
actions and reads the same post-action state as the HTTP connection.

Example transport code (supply your endpoint and action):
...
```
 ...
 A successful response has `ok: true`, a `result`, and
