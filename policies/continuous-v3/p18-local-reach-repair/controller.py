...
def place(name,p,v,exact=True):
    try:
        ...
        if 'too far away' not in str(error):
            raise
        ...
        if length<=0:
            raise
        reach=min(6.0,length)
        action('move',position={'x':p['x']-dx*reach/length,'y':p['y']-dy*reach/length})
        action('place',item=name,position=p,direction=direction(v),exact=exact)
...
def build():
    ...
    for orientation in [(1,0),(0,1),(-1,0),(0,-1)]:
        try:
            ...
            for machine, count in [(furnace,50),(inserter,30),(drill,50)]:
                fuel(machine,count)
...
print(json.dumps({'report':report+' Trace: '+json.dumps(trace)}))
