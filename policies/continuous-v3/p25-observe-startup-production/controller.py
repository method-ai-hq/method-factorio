...
def action(name, **fields):
    ...
    body = {'batch':fields['actions']} if name=='batch' else {'action': name, **fields}
    ...
    for member in result.get('results',[]):
        if isinstance(member.get('state'),dict):
            state=member['state']
...
def build():
    ...
    for orientation in [preferred]+[v for v in [(1,0),(0,1),(-1,0),(0,-1)] if v!=preferred]:
        try:
            ...
            belt_point={axis:math.floor(drill['drop_position'][axis])+.5 for axis in ['x','y']}
            inserter_point=point_add(belt_point,vector)
            action('batch',actions=[
                {'action':'place','item':'transport-belt','position':belt_point,'direction':direction(vector),'exact':True},
                {'action':'place','item':'burner-inserter','position':inserter_point,'direction':direction(vector),'exact':True},
                {'action':'place','item':'stone-furnace','position':point_add(inserter_point,vector,1.5),'direction':direction(vector),'exact':True}])
            belt=observed('transport-belt')
            inserter=observed('burner-inserter')
            if distance(belt['position'],belt_point)>.25:
                raise RuntimeError('Belt placement differs from expected tile')
            ...
            furnace=observed('stone-furnace')
            ...
            allocations=[(furnace,3),(inserter,2),(drill,4)]
            responses=action('batch',actions=[{'action':'insert','item':'coal','quantity':count,'target':target(machine)} for machine,count in allocations])
            if not isinstance(responses,list) or len(responses)!=len(allocations):
                raise RuntimeError('Fuel batch returned incomplete results')
            for response,(machine,count) in zip(responses,allocations):
                if not response.get('ok') or response.get('result',{}).get('inserted',0)<count:
                    raise RuntimeError('Fuel batch did not fill '+machine['name'])
